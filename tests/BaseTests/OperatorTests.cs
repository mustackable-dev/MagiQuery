...
public abstract class OperatorTests
{
    ...
    public void ApplyQuery_EqualsOperator_ShouldReturnEntry(string property, string value)
    {
        ...
        {
            OverrideDateTimeKind = Fixture.Provider == DataProvider.MongoDb ? DateTimeKind.Utc : null
        };
        ...
        if(Fixture.Provider == DataProvider.MongoDb && property.StartsWith("Contract."))
        {
        ...
        Assert.SkipWhen(
            property == "Strength" && Fixture.Provider != DataProvider.Runtime,
    ...
    public void ApplyQuery_GreaterThanOperator_ShouldReturnEntries(string property, string value, int expectedCount)
    {
        ...
        Assert.SkipWhen(
            Fixture.Provider==DataProvider.Sqlite && property == "Mana",
            "Operator not supported for type 'ulong' in Sqlite");
        
        Assert.SkipWhen(
            Fixture.Provider==DataProvider.Sqlite && property == "Contract.Details.Duration",
            "Operator not supported for type 'TimeSpan' in Sqlite");
        
        Assert.SkipWhen(
            Fixture.Provider==DataProvider.Sqlite && property == "DateOfConception",
        ...
        string? overrideCulture = null)
    {
        ...
        Assert.SkipWhen(
            Fixture.Provider == DataProvider.MySql,
            "\"StartsWith\" is not supported with this provider.");
        
        Assert.SkipWhen(
            Array.Exists([DataProvider.PostgreSql, DataProvider.MySqlPomelo], x=>x == Fixture.Provider) &&
        ...
        Assert.SkipWhen(
            Fixture.Provider==DataProvider.MongoDb && property == "DateOfConception",
            "Limited support for comparison on string conversions of DateTimeOffset in MongoDB");
        
        Assert.SkipWhen(
            Fixture.Provider==DataProvider.MongoDb && property == "FavouriteLetter",
        ...
        string? overrideCulture = null)
    {
        ...
        Assert.SkipWhen(
            Fixture.Provider != DataProvider.Runtime &&
        ...
        Assert.SkipWhen(
            Array.Exists(
                [DataProvider.SqlServer, DataProvider.MySqlPomelo], x=>x==Fixture.Provider) &&
                property == "Salary",
        ...
        string? overrideCulture = null)
    {
        ...
        Assert.SkipWhen(
            Array.Exists([DataProvider.SqlServer, DataProvider.MySql], x=>x==Fixture.Provider),
    ...
    public void ApplyQuery_NullOperators_ShouldReturnEntries() 
    {
        ...
        Assert.SkipWhen(
            Fixture.Provider == DataProvider.MongoDb,
